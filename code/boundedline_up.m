function varargout = boundedline_up(varargin)
%
...
% Edited by Dmitry Ostrovsky to plot only upper confidence bounds.
...
for ix = 1:length(x)
    ...
    for iln = 1:nline
        ...
        if isequal(sz, [nx 2])
            lo{iln} = lineval - 0*err2{iln}(:,1)';
            hi{iln} = lineval + err2{iln}(:,2)';
        elseif isequal(sz, [nx 1])
            lo{iln} = lineval - 0*err2{iln}';
